...
function joinGame(code) {
  ...
  get(gameRef).then(snapshot => {
    ...
    if (data.player2) {
      alert("משחק זה כבר מלא!");
      return;
    }

    // הוספת שחקן 2 ועדכון מצב המשחק
    ...
    console.error("שגיאה ב-Firebase:", err);
    alert("שגיאה בטעינת המשחק, נסה שוב.");
  });
}
